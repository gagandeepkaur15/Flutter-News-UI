...
class _FeedState extends State<Feed> {
  ...
  Future<List<dataClass>> getData() async {
    ...
    final res = await http.get(
        Uri.parse("https://api.dev.agrishots.in/articles?page=1&perPage=758"));
    if (res.statusCode == 200) {
      Map<String, dynamic> resp = jsonDecode(res.body.toString());
      resp.entries.forEach((element) {
        if (element.key == "data") {
          print(element.value);
          List data = element.value;
          data.forEach((element) {
            postList.add(dataClass.fromJson(element));
          });
        }
      });
    ...
    //   .then((value) {
    // Map<String, dynamic> res = jsonDecode(value.body.toString());
    // if (value.statusCode == 200) {
    //   // for (Map i in res) {
    //   //   postList.add(dataClass.fromJson(i));
    //   // }
    //   res.entries.forEach((element) {
    //     if (element.key == "data") {
    //       List data = element.value;
    //       data.forEach((element) {
    //         postList.add(dataClass.fromJson(element as Map<String, dynamic>));
    //         print(postList.length);
    //       });
    //     }
    //   });
